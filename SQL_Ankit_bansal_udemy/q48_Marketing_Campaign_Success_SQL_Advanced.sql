...
------------------------------- Marketing Campaign Success SQL Advanced -------------------------------
------------------------------- Marketing Campaign Success SQL Advanced -------------------------------
...
marketing campaign where they see call-to-actions for more in-app purchases.
...
made one or multiple purchases one the first day do not count, nor do we count users that overtime purchase 
...
with cte1 as (
	select *, 
	dense_rank() over(partition by user_id order by created_at) as d_rn
...
--	where user_id in (11,14, 25)
    ...
    select count(distinct c2.user_id) as user_cnt 
	from cte2 as c2
    left join cte3 as c3
    on c2.user_id = c3.user_id and c2.product_id =  c3.product_id
